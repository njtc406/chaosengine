...
const defaultMaxTaskChannelNum = 100000
...
func (c *Concurrent) AsyncDoByQueue(queueId int64, fn func() bool, cb func(err error)) {
	...
	default:
		log.SysLogger.Warn("tasks channel is full")
		if cb != nil {
...
func (c *Concurrent) Close() {
	...
	log.SysLogger.Info("wait close concurrent")

	c.dispatch.close()

	log.SysLogger.Info("concurrent has successfully exited")
}
