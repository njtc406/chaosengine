...
import (
	...
	"github.com/njtc406/chaosengine/engine1/systemmodule/httpmodule/auth"
...
func (hs *HttpModule) OnInit() chaoserrors.CError {
	// 替换验证器(这个东西之后再看用哪个版本)
	//*(binding.Validator.Engine().(*validator.Validate)) = *validate.Validator
	// 默认日志输出
	gin.DefaultWriter = io.MultiWriter(hs.logger.WriterLevel(chaoslog.InfoLevel))       // 设置默认日志输出为info级别
	gin.DefaultErrorWriter = io.MultiWriter(hs.logger.WriterLevel(chaoslog.ErrorLevel)) // 设置默认错误日志输出为error级别
	// 运行模式
...
func (hs *HttpModule) OnStart() error {
	if !atomic.CompareAndSwapUint32(&hs.running, 0, 1) {
		return chaoserrors.NewErrCode(chaoserrors.IsRunning, "http service is running", nil)
	}
	hs.wg.Add(1)
	go hs.run()
...
func NewHttpModule(conf *Conf, logger chaoslog.ILogger, systemMod string) *HttpModule {
	return &HttpModule{
		handler:   gin.New(),
		wg:        new(sync.WaitGroup),
		conf:      conf,
